...
class TietoaPipeline:

    # Database setup
    conn = sqlite3.connect('tietoa.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `tietoa`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM tietoa WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO tietoa (date, title, link, content)"
